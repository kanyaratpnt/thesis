  ...
  const UniformBlock = () => {
    ...
    return (
      <div className="pdUniformBox" id="uniform-details">
        ...
        {(() => {
          const allItems = project.uniform_items || [];
          // สรุปตาม uniform_category รวมทุก gender และทุก level
          ...
          for (const item of allItems) {
            ...
            const prev = summaryMap.get(cat) || { remaining: 0, total: 0, image_url: null };
            summaryMap.set(cat, {
          ...
          return (
            <div style={{
              ...
              {summaryItems.map(([cat, info]) => {
                ...
                const pct = info.total > 0 ? Math.round(((info.total - info.remaining) / info.total) * 100) : 0;
                return (
                  <div key={cat} style={{
                    flex: "1 1 130px",
                    background: "#fff",
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {info.image_url ? (
                        <img src={info.image_url} alt={cat}
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 20, fontWeight: 800, color: info.remaining > 0 ? "#f59e0b" : "#16a34a", lineHeight: 1 }}>
                          {info.remaining > 0 ? `ขาด ${info.remaining}` : "ครบแล้ว!"}
                        </div>
                        <div style={{ fontSize: 11, color: "#94a3b8" }}>จากทั้งหมด {info.total} ตัว</div>
                      </div>
                    </div>
                    {/* progress bar */}
                    <div style={{ height: 4, background: "#e2e8f0", borderRadius: 99, overflow: "hidden", marginTop: 4 }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: pct >= 100 ? "#16a34a" : "#5285e8", borderRadius: 99, transition: "width 0.4s" }} />
                    </div>
                    <div style={{ fontSize: 10, color: "#94a3b8", textAlign: "right" }}>{pct}% ได้รับแล้ว</div>
                  </div>
                );
              })}
            </div>
